					...
					<c:choose>
						<c:when test="${easybuyCollect.productId==product.id }">
							<div class="d_care"
								...
								onclick="addFavoriteDeatil(this,'${product.id}')">
				...
				<div class="des_join">
					<div class="j_nums">
						<input type="text" value="1" name="quantity" class="n_ipt"
							maxlength="3" /> <input type="button" value=""
							onclick="addUpdate(jq(this));" class="n_btn_1" /> <input
							type="button" value="" onclick="jianUpdate(jq(this));"
							class="n_btn_2" /> <input type="hidden" name="productStock"
							value="${product.stock}" />
					</div>
					...
					<c:choose>
						...
						<c:otherwise>
							<span class="fl"> <img
								src="${ctx}/statics/images/j_car.png" onclick="addCart();" />
							</span>
						</c:otherwise>
					</c:choose>
				</div>
