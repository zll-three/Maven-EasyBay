                ...
                <ul class="cate_list">
                    <c:forEach items="${productList}" var="productList">
                        <li>
                            ...
                            <div class="name"><a href="${ctx}/pro/queryProductDeatil/${productList.id}">${productList.name}</a></div>
                            <div class="carbg">
                            ...
                            <c:when test="${flag!=productList.id }">
                             <a href="javascript:void(0);" class="ss" onclick="addFavorite(this,'${productList.id}')">收藏</a>
                            </c:when>
                            <c:otherwise>
                            <a href="javascript:void(0);" class="ss" style="background-image: url(${ctx}/statics/images/heart_h.png)" onclick="addFavorite(this,'${productList.id}')">收藏</a>
                            </c:otherwise>
                            </c:choose>                  
                                <a href="javascript:void(0);" class="j_car" onclick="addCartByParam('${productList.id}',1);">加入购物车</a>
                            </div>
                        </li>
                    </c:forEach>
                </ul>
...
function clickimg(id) {
	location.href = contextPath+"/pro/queryProductDeatil/"+id;
}
